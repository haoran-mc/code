...
/*删除某个值的结点*/
LinkList ListDeletex(const LinkList &pNode, int val) {
    LinkList Find;
    InitList(Find);
    Node *res  = Find;
    Node *seek = pNode -> next;
    while (seek) {
        while (seek && seek -> data == val) {
            seek = seek -> next;
        }
        if (seek) {
            InsertListEnd(Find, seek -> data);
            seek = seek -> next;
        }
    }
    return res;
...
int main()
{
    ...
    printf("input the number of number of list: ");
    int n;
    scanf("%d", &n);
    for (int i = 0; i < n; ++i) {
        int num;
        scanf("%d", &num);
        InsertListEnd(pNode, num);
    }
    printf("input the number you want to delete: ");
    int val;
    scanf("%d", &val);
    printf("\n");
    LinkList del = ListDeletex(pNode, val);
    outputList(del);
