\message{ !name(latexMathBasics.tex)}% 导言区
...
\message{ !name(latexMathBasics.tex) !offset(-3) }
    ...
    \begin{equation} % 在equation环境中的公式会自动编号
    ...
    公式的编号与交叉引用也是自动实现的，在排版中，要习惯于采用自动化的方式处理诸如图、比表、公式的编号与交叉引用
    再如公式\ref{eq:pol}
    \begin{equation}
...
\message{ !name(latexMathBasics.tex) !offset(-117) }
