...
int main()
{
    ...
    int count = 0;
    while (index != -1) {
        //printf("%05d, %d, %05d\n", index, link[index].data, link[index].nxt);
        ++count;
        index = link[index].nxt;
    }
    //printf("count = %d, N = %d\n", count, N);
    ...
    for (int i = 0; i < count; ++i) {
        if (i >= count-count%K) {
            link[index].flagEnd = true;
            //printf("%d %d %d %d\n", i, count-count%K, index, link[index].data);
        }
        index = link[index].nxt;
    }
    index = temp;
    while (index != -1) {
        //printf("%05d, %d, %05d, %d\n", index, link[index].data, link[index].nxt, link[index].flagEnd);
        index = link[index].nxt;
    }
